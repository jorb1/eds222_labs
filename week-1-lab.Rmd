...
loans <- loans_full_schema |>
  mutate(homeownership = str_to_lower(homeownership)) |>
...
ggplot(data = loans_sum) +
  geom_col(aes(x = homeownership, y = n),
           fill = "seagreen") + 
  theme_minimal()
...
loans_table <- loans %>%
  ...
  ungroup() %>%
  bind_rows(summarize_all(., list(~ if(is.numeric(.)) sum(., na.rm = TRUE) else "Total")))
...
loans |>
  ggplot() +
  geom_bar(aes(x = homeownership, fill = application_type), position = "dodge")
```
