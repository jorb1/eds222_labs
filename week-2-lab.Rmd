...
```{r}
lter <- lterdatasampler::pie_crab


ggplot(data = lter) +
  geom_point(aes(x= latitude, y = size)) +
  theme_minimal() 
```
...
```{r}
cor(x = lter$latitude, y = lter$size)
```
...
```{r}
#lm(lter$size ~ lter$latitude) # y ~ x (y is a function of x)

summary(lm(size ~ latitude, lter))
```
...
```{r}
ggplot(data = lter, aes(x= latitude, y = size)) +
  geom_point() +
...
```{r}
lter2 <- lter
crab_lm <- lm(size ~ latitude, lter)

new_crabs <- lter2 |>
  mutate(predited_size = predict(crab_lm),
         size_residul = resid(crab_lm)) |>
  view()
...
ggplot(new_crabs, aes(x=latitude, y = size_residul)) +
  geom_point() +
