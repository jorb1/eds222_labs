...
drinking_water <- drinking_water |>
  mutate(violation = ifelse(health > 0, 1, 0))
```
...
ggplot(drinking_water, aes(pcthisp, violation)) +
  geom_point() +
  geom_point(data = drinking_water_binned,
             color = "seagreen")
```
...
#logit(p) = 
logitp_me <- (-2.422124762 + 0.004398142*c(0, 50, 100))
      
p <- inv_logit(logitp_me)
```
...
```{r}
logitp_middle <- (-2.422124762 + 0.004398142*c(10, 45, 80))
logitp_increase <- (-2.422124762 + 0.004398142*c(20, 55, 90))

change <- inv_logit(logitp_increase - logitp_middle)
```
...
ggplot(likelihood_dem, aes(beta0, beta1, fill = negloglik)) +
  geom_raster() +
  geom_point(x = -2.42, y = 0.00439, color = "coral")
