...
# 2. size~latitude has a higher r value, so its a better fit
```
...
```{r}
sse <- sum((resid(lm(size ~ latitude, pie_crab)))^2)
sse
...
```{r}
ggplot(data = mpg, aes(x= year_fct, y = hwy)) +
  geom_boxplot()
```
...
3.  What does the coefficient estimate on the 2008 vintage indicator variable tell you, in words? **Answers:**

```{r}
summary(lm(hwy ~ year_fct, mpg))

# Beta 0 is the reference level(intercept). The value at the reference level. Beta 1 is representing the change - what is the difference to the refernce level (beta 0). 
# The intercept tells us the difference between these variables is tiny
```
...
```{r}
hwy_lm <- lm(hwy ~ year_fct + displ, mpg)
summary(hwy_lm)
```
...
ggplot(mpg, aes(displ, hwy, color = year_fct)) +
  geom_point() +
  geom_line(aes(y = .fitted), augment(hwy_lm)) +
  theme_bw()
```
